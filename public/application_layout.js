$(document).ready(function() {
	...
	$('.getstarted').click(function(e) {
		...
		if ($("div.getstarted").attr("id")!="active") {
			...
			'<form id="new_smb" class="new_smb" method="post" enctype="multipart/form-data" action="/smbs" accept-charset="UTF-8">' +
			...
			'</div>' +
			'<div class="field">' +
			'<input id="smb_password" type="password" value="" autocomplete="off" placeholder="Password" name="smb[password]" style="width: 156px;"><br/>'+
			'</div>'+
			'<div class="field">'+
			'<input id="smb_password_confirmation" type="password" name="smb[password_confirmation]" placeholder="Password confirmation" style="width: 156px;"><br/>'+
			'</div>'+
			...
			// $('input#smb_email').blur(function() {  // 
				...
				// }
			// });
			// $('input#smb_password').blur(function() {  // 
				...
				// }
			// });
			// $('input#smb_password_confirmation').blur(function() {  // 
				// if ($(this).val() == '' || $(this).val() == $(this).attr('placeholder') || $(this).val() != $('input#smb_password').val()) { // maybe also add regular expression test.
					// $(this).css('outline', '1px solid red');
	...
	$('.user_dropdown#change_pin').live('click', function() {
		$('body').append(
			...
			'<input type="text" id="new_pin" style="width:250px" placeholder="Enter a new 4-digit PIN">'+
			'</div>'+
			'<div class="field" style="margin-bottom:10px">'+
			'<input type="password" id="pin_password" style="width:250px" placeholder="Please enter you password">'+
			'</div>'+
		...
		$('.change_pin_button#change').click(function() {
			...
			container.append(
				'<div id="loading_container" style="text-align:center;margin-top:30px"><img src="/loading.gif"/></div>'
			);
			$.ajax({
				url: '/smbs/'+ user_token +'/change_pin',
				data: 'new_pin=' + $("input#new_pin").val() + '&' + 'pin_password='+$("input#pin_password").val(),
				dataType: 'json', 
				type: 'POST',
				success: function (j) {
					if (j.ok) {
	...
	$('.user_dropdown#change_password').live('click', function() {
		$('body').append(
			...
			'<img class="close_change" src="/x_button.png">'+
			'<div class="field" style="margin-bottom:10px">'+
			'<input type="password" style="width:250px" placeholder="Please enter your old password" id="old_password" name="old_password">'+
			'</div>'+
			'<div class="field" style="margin-bottom:10px">'+
			'<input type="password" style="width:250px" placeholder="Specify a new password" id="new_password" name="new_password">'+
			'</div>'+
			'<div class="field" style="margin-bottom:10px">'+
			'<input type="password" style="width:250px" placeholder="Enter your new password again" id="new_password_confirmation" name="new_password_confirmation">'+
			'</div>'+
			'<div style="padding-top:15px">'+
		...
		$('.change_password_button#change').click(function() {
			if ($('input#old_password').val() == '') {
				$('input#old_password').css('outline', '1px solid red');
				$('label.login_notice#change_password').text('Please enter your old password.');
				setTimeout(function() {
					$('input#old_password').css('outline', '');
					$('label.login_notice#change_password').text('');
				}, 800);
			} else {
				$.ajax({
					url: '/smbs/check_old_password',
					data: 'password_reset_email=' + $("input#user_email").val() + '&' + 'old_password=' + $("input#old_password").val(),
					dataType: 'json', 
					type: 'POST',
					success: function (j) {
						if (!j.ok) { // password doesn't match
							...
							if ($('input#new_password').val() == '' && $('input#new_password_confirmation').val() == '') {
								$('input#new_password').css('outline', '1px solid red');
								$('input#new_password_confirmation').css('outline', '1px solid red');
								$('label.login_notice#change_password').text('Please fill in required fields.');
								setTimeout(function() {
									$('input#new_password').css('outline', '');
									$('input#new_password_confirmation').css('outline','');
									$('label.login_notice#change_password').text('');
								}, 800);
							} else if ($('input#new_password').val() == '') {
								$('input#new_password').css('outline', '1px solid red');
							...
							} else if ($('input#new_password_confirmation').val() == '') {
								$('input#new_password_confirmation').css('outline', '1px solid red');
								...
								if ($('input#new_password').val() != $('input#new_password_confirmation').val()) {
									...
									$.ajax({
										...
										data: 'password_reset_email=' + $("input#user_email").val() + '&' + 'old_password='+ $('input#old_password').val() + '&' + 'new_password=' + $("input#new_password").val() + '&' + 'new_password_confirmation=' + $("input#new_password_confirmation").val(),
										dataType: 'json', 
										type: 'POST',
										success: function (k) {
											if (k.ok) {
												$('input#old_password, input#new_password, input#new_password_confirmation').attr('disabled', true);
												$('label.login_notice#change_password').css('color', '#66cc00').text('Password has been updated!');
												setTimeout(function() {
													$('.close_change').click();
												}, 800)
											} else {
												$('label.login_notice#change_password').text('Something wrong. Please retry. (Check your old password)');
												setTimeout(function() {
													$('label.login_notice#change_password').text('');
												}, 800)
											}
	...
	$(".login").click(function(e) {
  		...
  		if ($("div.login").attr("id")!="active") {
  			$(".header_nav").append('<div class="login_dropdown" hovered="false" >'+ '<form class="login_form" method="post" action="/sessions" data-remote="true" accept-charset="UTF-8">'+
									...
									'</div>'+
									'<div class="field">'+
									'<input id="password" type="password" value="" placeholder="Password" name="password" style="width: 135px;">'+
									'</div>'+
									'<div class="field">'+
									...
									'</div>'+
									'</form>' +
									'<a style="cursor:pointer;" class="forgot">Forgot your password?</a><br/>' +
			...
			$("input.login_button").click(function(event) {
				if ($('input#email[type=text]').val() == '' && $('input#password').val() == '') {
					//event.preventDefault();
				...
				} else if ($('input#email[type=text]').val() == '') {
					//event.preventDefault();
					...
					$.ajax({
						url: '/sessions/create',
						data: 'email=' + $('input#email[type=text]').val() + '&' + 'password=' + $('input#password').val(),
						dataType: 'json',
						type: 'POST',
						success: function(j) {
							if (j.ok == false) {
	...
	//if ($('[placeholder]').val() == $('[placeholder]').attr('placeholder')) $('[placeholder]').val(''); // maybe not right TODO
	//if ($('[placehold]').val() == $('[placehold]').attr('placehold')) $('[placehold]').val(''); // maybe not right TODO
